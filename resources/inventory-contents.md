...
The game uses one shared ID space for all of these categories, split into ranges (item IDs start at `0x0000`, equipment at `0x1000`, magicks at `0x3000`, and so on). This sheet is the decoder ring for "Content ID X" wherever it shows up in other game data, loot tables, shop lists, action rewards, and more. It matches The Insurgent's Toolkit's Inventory Editor, which lets you add or remove any of these categories directly in a live save.
